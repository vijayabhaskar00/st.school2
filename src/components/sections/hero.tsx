...
const BrandReelPlayer = dynamic(
  () => import("@/components/remotion/brand-reel-player").then((m) => m.BrandReelPlayer),
  {
    ssr: false,
    loading: () => (
      <div className="aspect-[8/5] w-full animate-pulse rounded-2xl border border-white/10 bg-ink-elevated/60" />
    ),
  },
);
    ...
    <section className="relative overflow-hidden pt-32 pb-20 lg:pt-40">
      ...
      <Container className="grid items-center gap-16 lg:grid-cols-[1.05fr_1fr] lg:gap-10">
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="absolute -inset-4 -z-10 rounded-[2rem] bg-gradient-to-br from-violet/20 via-transparent to-coral/20 blur-2xl" />
